...
driver와 executor가 각각 무엇을 하는지 흐름으로 풀어 보면 다음과 같습니다.
...
`debug-common-pitfalls.md` §1 "직접 만들지 말 것"이 가리키는 지점이 바로 이 표입니다. 위 값들은 모두 **TorchDistributor가 task 배치 시점에 계산**합니다. 사용자가 노트북에서 `os.environ["MASTER_ADDR"]=...`로 미리 잡으면 task별로 계산된 정확한 값을 덮어쓰게 되어 NCCL rendezvous가 깨집니다.
...
`02-script-based/README.md:56`과 `03-custom-package-script-based/README.md:20`에서 언급된 현상입니다(DBR 17.3 LTS ML, g5.12xlarge에서 관찰). 근본 원인은 확정되지 않았지만 가능한 가설은 다음과 같습니다.
...
본 쿡북이 채택한 회피책은 **launcher × topology 별로 노트북을 분리**해 매번 fresh Python interpreter로 시작하는 것입니다. 같은 노트북에서 `dbutils.notebook.exit` 후 재실행해도 되지만 노트북 분리가 더 깔끔합니다.
...
`local_mode=False` 학습 중에 Spark UI의 Jobs 탭을 보면 **barrier stage** 하나가 떠 있습니다. task 수는 `M*N`이고, 각 task의 stderr가 executor log로 흘러 들어갑니다. worker 노드의 NCCL/PyTorch 로그를 보려면 거기서 확인합니다(`debug-common-pitfalls.md`의 "Worker 로그 읽는 법" 섹션 참고).
