...
본 쿡북에서 반복적으로 등장하는 핵심 개념을 한 문서에 모읍니다.
...
본 쿡북의 기준 모델([recommender-baseline.md](recommender-baseline.md))은 가장 큰 셀에서도 수 GB 임베딩 테이블 + 작은 MLP라 단일 GPU에 들어갑니다. 따라서 **DDP만으로 충분**합니다. FSDP(모델 샤딩)는 다루지 않습니다.

> 토폴로지(1×1 / 1×N / M×N)는 **GPU 개수**의 문제고, DDP는 **여러 GPU에 같은 모델을 복제해 데이터 병렬 처리**하는 *전략*입니다. 1×N과 M×N 모두 DDP를 쓰며, 차이는 GPU가 노드 안에만 있는지 노드를 가로지르는지뿐입니다.

## TorchDistributor `local_mode`: driver vs worker

TorchDistributor의 `local_mode` 플래그는 **분산 학습 프로세스를 어디에서 띄울지**를 결정합니다.
...
- `local_mode=True`는 "driver에서 학습한다"는 뜻입니다. Single-node 클러스터(driver만 있고 worker=0)나 driver에 GPU가 있는 클러스터에서 사용. TorchDistributor가 driver 머신 안에서 `num_processes`개의 Python 프로세스를 띄우고 NCCL로 묶습니다.
- `local_mode=False`는 "worker에서 학습한다"는 뜻입니다. Spark가 worker 노드들에 child 프로세스를 분배합니다. driver는 학습에 끼지 않고 RPC 코디네이션만 합니다 — 그래서 multi-node에서 driver-side `log_system_metrics=True`만 켜면 idle metric만 잡힙니다 ([`mlflow-tracking.md §1`](mlflow-tracking.md)).
...
함정
- single-node 클러스터에서 `local_mode=False`로 부르면 worker가 없어 실패합니다.
- multi-node 클러스터에서 `local_mode=True`로 부르면 worker 노드가 놀고 driver 1대만 학습합니다.
- `local_mode=False`의 child는 다른 노드의 fresh Python으로 시작하므로 driver 자격증명·import를 자동 상속하지 않습니다 ([`common-pitfalls.md §2`, `§2-1`](common-pitfalls.md)).
...
본 쿡북의 매핑 (`01-notebook-based/04-train_pytorch_lightning_single_node.ipynb`, `05-train_pytorch_lightning_multi_node.ipynb`):
...
- TorchDistributor child는 노트북과 별개의 fresh Python으로 시작합니다. LightningModule/DataModule, callback import는 모두 `train_fn` 안에서 다시 정의·import 해야 합니다 ([`common-pitfalls.md §2`](common-pitfalls.md)).
