...
같은 학습 launcher라도 single-node와 multi-node 노트북은 **별도 클러스터**에서 띄우는 것을 권장합니다. autoscaling·single-node 토글 설정이 다르기 때문입니다. 상세 인스턴스 비교는 [`00-foundations/env-cluster-recipes.md`](../00-foundations/env-cluster-recipes.md)를 참고하세요.
...
H100/A100을 쓰려면 `p4d.24xlarge` 또는 H100 인스턴스로 교체합니다 ([`env-cluster-recipes.md`](../00-foundations/env-cluster-recipes.md)).
...
- multi-node throughput이 single-node와 비슷하다면 inter-node bandwidth 병목입니다 ([`debug-common-pitfalls.md §10`](../00-foundations/debug-common-pitfalls.md)).
- GPU util이 10% 미만이면 DataLoader 병목이므로 `num_workers`와 `pin_memory`를 점검합니다.
- MLflow UI의 System Metrics 탭에서 GPU 차트가 비어 있다면 `nvidia-ml-py`가 미설치이거나 multi-node에서 driver-side만 켠 경우입니다 ([`debug-common-pitfalls.md §9`](../00-foundations/debug-common-pitfalls.md)).
...
- 멀티 노드(03, 05) 노트북에서는 driver에서 worker로 `db_host`와 `db_token`을 명시 전달해야 합니다 ([`00-foundations/debug-common-pitfalls.md#2-1`](../00-foundations/debug-common-pitfalls.md)).
