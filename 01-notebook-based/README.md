...
같은 학습 launcher라도 single-node / multi-node 노트북은 **별도 클러스터**에서 띄우는 것을 권장합니다 (autoscaling·single-node 토글 설정이 다름). 상세 인스턴스 비교는 [`00-foundations/env-cluster-recipes.md`](../00-foundations/env-cluster-recipes.md).
...
H100/A100을 쓰려면 `p4d.24xlarge` 또는 H100 인스턴스로 교체 ([`env-cluster-recipes.md`](../00-foundations/env-cluster-recipes.md)).
...
- **System metrics**(GPU 활용도, 메모리, I/O)는 `mlflow.start_run(..., log_system_metrics=True)`로 수집됩니다.
  ...
  - **Multi-node(03, 05)**: driver는 학습에 참여하지 않으므로 driver-side만 켜면 idle 메트릭만 잡힙니다. 03·05의 학습 함수는 **rank-0 worker에서 `mlflow.start_run(run_id=..., log_system_metrics=True)`로 attach**해 worker 노드의 GPU 메트릭을 함께 기록합니다 ([`00-foundations/ops-mlflow-tracking.md §1`](../00-foundations/ops-mlflow-tracking.md)).
...
- multi-node throughput 이 single-node와 비슷 → inter-node bandwidth 병목 ([`debug-common-pitfalls.md §10`](../00-foundations/debug-common-pitfalls.md)).
- GPU util 이 10% 미만 → DataLoader 병목. `num_workers` / `pin_memory` 점검.
- MLflow UI System Metrics 탭에서 GPU 차트가 비어 있음 → `nvidia-ml-py` 미설치 또는 multi-node에서 driver-side만 켠 경우 ([`debug-common-pitfalls.md §9`](../00-foundations/debug-common-pitfalls.md)).
...
- 멀티 노드 (03, 05) 노트북에서는 driver→worker로 `db_host`/`db_token`을 명시 전달해야 합니다. [`00-foundations/debug-common-pitfalls.md#2-1`](../00-foundations/debug-common-pitfalls.md)
- Serverless GPU는 multi-node를 지원하지 않습니다. 03, 05는 반드시 Classic GPU 클러스터에서 실행 ([`env-databricks-environments.md`](../00-foundations/env-databricks-environments.md)).
