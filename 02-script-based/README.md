...
> 학습 로직을 `.py` 모듈로 분리하고, 노트북은 driver(파라미터 구성·실행·결과 확인)만 담당하는 패턴입니다.

## 🧭 파일 구성

모듈과 노트북이 어떻게 짝을 이루는지 보여 줍니다.
...
TorchDistributor `.run`에 module-level 함수를 직접 넘기면 cloudpickle이 module reference로 직렬화하고, child 프로세스의 fresh `sys.path`에는 SCRIPT_DIR이 없어 import가 실패합니다. 이를 회피하기 위해 노트북에서 **inline thin wrapper**를 정의해 by-value pickling을 받게 하고, wrapper 내부에서 `sys.path`를 보강한 뒤 lazy import합니다.
...
> 패키지로 묶어 `%pip install`하는 형태가 필요하면 [`03-custom-package-script-based/`](../03-custom-package-script-based/README.md)으로 넘어가세요.

## 🔀 매트릭스

같은 `train_fn`(또는 `fit`)을 세 launcher × 세 토폴로지로 호출합니다.
...
> 한 노트북 세션에서 `TorchDistributor.run`을 연속 호출하면 driver의 py4j callback channel이 단절되는 현상이 관찰되어(DBR 17.3 LTS ML, g5.12xlarge) launcher × topology별로 노트북을 분리했습니다. 토폴로지를 비교할 때는 같은 클러스터에서 노트북을 차례로 detach한 뒤 다음 노트북을 attach해 실행하세요.
>
> Accelerate 행은 노트북 한 개(08)로 1×1/1×N/M×N을 모두 커버합니다. `accelerate launch`가 가시 GPU 수를 자동 감지하므로 클러스터 사양만 바꿔도 동일 노트북이 그대로 동작합니다. 호출은 `subprocess.Popen` + `shlex.split` 패턴을 사용하며, 자식 프로세스에 `DATABRICKS_HOST/TOKEN`을 명시 주입하고, Py4J keepalive 스레드로 장시간 학습 중 gateway 단절을 막고, non-zero return 시 `dbutils.notebook.exit`로 실패 종료합니다.

## 🖥️ 클러스터 세팅

토폴로지별 클러스터 토글 차이는 다음과 같습니다.
...
01-notebook-based와 코드·데이터·launcher 설정이 모두 동일합니다. 차이는 import 방식뿐이므로 결과도 같아야 합니다.
...
설치 가능 wheel로 패키징한 버전은 [`03-custom-package-script-based/`](../03-custom-package-script-based/README.md)에서 이어집니다.
