...
설치 가능한 파이썬 패키지(`recommender_pkg`). 03-custom-package-script-based의 노트북들이 wheel install 후 import한다.
...
또는 cluster 라이브러리로 부착해서 모든 노트북이 자동 import 가능하게 한다.

## 📝 02-script-based와의 코드 동등성

`model.py`, `torch_distributor_trainer.py`, `lightning_trainer.py`의 **함수 시그니처와 동작은 02-script-based와 동일**해야 한다. 차이는 import 경로 (`recommender_pkg.model` vs 동일 폴더의 `model`) 뿐.
