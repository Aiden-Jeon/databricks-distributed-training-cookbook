...
Databricks에서 **추천 모델(Two-Tower MLP)**을 분산학습하는 패턴을 **실행 방식 × 컴퓨트 토폴로지** 매트릭스(4×3 = 12셀)로 정리한 쿡북.
...
- 기준 모델은 `00-foundations/recommender-baseline.md` 한 곳에 정의. 셀별로 임베딩 크기와 사용자/아이템 수만 스케일.
...
| 데이터 | 합성 user-item interaction → Delta Lake → Unity Catalog Volumes parquet shard |
| 실험 추적 | MLflow (autolog) |
| 패키징 | uv (로컬), Databricks 노트북/Repos |

**사용 안 함**: FSDP (모델 샤딩 불필요), DeepSpeed, TorchRec, HuggingFace Transformers/TRL/PEFT (LLM fine-tuning 스택은 이번 쿡북 스코프 밖).
...
│   ├── 02-single-node-multi-gpu/          # @distributed + DDP
│   └── 03-multi-node-multi-gpu/           # TorchDistributor + DDP
...
| 셀 유형 | 실행 메커니즘 |
|---------|---------------|
| 1×1 GPU 노트북 | 노트북 셀에서 PyTorch 학습 루프 직접 호출 |
...
| Lightning | `Trainer(num_nodes=M, devices=N, strategy="ddp").fit(...)` |
...
- 각 셀의 학습 시간은 합성 데이터로 **15분 이내**를 목표로 한다. 데이터 행 수와 임베딩 크기는 셀별로 차등.
