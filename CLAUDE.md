...
- 기준 모델은 `00-foundations/recommender-baseline.md` 한 곳에 정의. 노트북 섹션별로 임베딩 크기와 사용자/아이템 수만 스케일.
...
|----------|---------|---------|---------|
| TorchDistributor | standard PyTorch loop | `TorchDistributor(num_processes=N, local_mode=True).run(fn, ...)` | `TorchDistributor(num_processes=M*N, local_mode=False).run(fn, ...)` |
...
- 각 노트북 섹션의 학습 시간은 합성 데이터로 **15분 이내**를 목표로 한다. 데이터 행 수와 임베딩 크기는 섹션별로 차등.
