...
- 기준 모델은 `00-foundations/concepts-recommender-baseline.md` 한 곳에 정의. **데이터셋·모델 config 모두 단일**(ML-25M, 단일 CONFIG)이고, 토폴로지 차이는 launcher 설정·batch_size로만 표현.
...
├── 00-foundations/                                 # 그룹 prefix: concepts- / env- / data- / ops- / debug-
├── 01-notebook-based/                              # 행 1: 노트북 only
│   ├── 00-setup.ipynb
│   ├── 01-data_prep.ipynb
│   ├── 02-train_torch_distributor_1x1_1xN.ipynb        # 1×1 / 1×N
│   ├── 03-train_torch_distributor_MxN.ipynb            # M×N
│   ├── 04-train_pytorch_lightning_1x1_1xN.ipynb        # 1×1 / 1×N
│   └── 05-train_pytorch_lightning_MxN.ipynb            # M×N
...
- 모델은 `00-foundations/concepts-recommender-baseline.md`의 Two-Tower MLP를 기준으로 한다. 다른 추천 모델(SASRec, BERT4Rec 등)이나 LLM으로 확장하지 않는다.
