...
| 5 | `01-notebook-based/04`, `05` (Lightning) | 같은 모델·데이터로 Lightning Trainer 비교 | MLFlowLogger 자동 동작, EarlyStopping callback |
...
- **02 → 03**: 같은 cluster에서도 child가 다른 컨텍스트(Accelerate subprocess, Job task)로 도는 시나리오에서 sys.path 패턴이 깨짐. wheel install이 이를 일관되게 해결 ([`00-foundations/library-management.md`](00-foundations/library-management.md)).
