...
02·03 행은 launcher × topology별로 노트북을 분리했습니다 (한 노트북에서 `TorchDistributor.run`을 연속 호출 시 py4j callback이 단절되는 현상 회피). Accelerate는 단일 노트북에서 가시 GPU 수를 자동 감지해 1×1/1×N/M×N을 모두 다룹니다 (subprocess.Popen). 노트북 목록은 [`02-script-based/README.md`](02-script-based/README.md), [`03-custom-package-script-based/README.md`](03-custom-package-script-based/README.md).
...
> **표기 규칙**: `M × N` 에서 **M = 노드 수**, **N = 노드당 GPU 수**. 학습 process(rank) 총 개수는 `world_size = M × N` 입니다.
...
01-notebook-based 행은 `1x1_1xN` 노트북에서 1×1 / 1×N을 섹션으로 함께 다루고 `MxN` 노트북에서 M×N을 다룹니다. 02·03 행은 launcher × topology 별로 노트북이 분리되어 파일명에 `1x1` / `1xN` / `MxN` 이 들어갑니다.
...
1. 처음이면 [`00-foundations/`](00-foundations/README.md)부터 읽습니다.
   - 기준 모델 정의, Classic GPU 환경, 클러스터 권장 사양, 데이터 로딩, MLflow, UC Volumes, TorchDistributor 내부 동작, 라이브러리·인증 관리, 흔한 함정.
...
분산 학습 자체는 익숙하지만 Databricks 위에서 처음 돌리는 경우, 다음 순서로 학습 시간을 최소화하면서 점진적으로 확장할 수 있습니다.
...
각 단계의 가치:
- **01 → 02**: 노트북 셀 안의 학습 코드를 모듈화하면 by-value pickling 한계와 import 경로 함정을 만남. PoC에서 운영 패턴으로 전환할 때 반드시 경험.
- **02 → 03**: 같은 cluster에서도 child가 다른 컨텍스트(Accelerate subprocess, Job task)로 도는 시나리오에서 sys.path 패턴이 깨짐. wheel install이 이를 일관되게 해결 ([`00-foundations/env-library-management.md`](00-foundations/env-library-management.md)).

각 행의 학습 시간 ballpark은 행 README의 "기대 결과" 표 참고.
