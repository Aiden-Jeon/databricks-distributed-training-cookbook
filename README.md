...
Databricks 환경에서 **추천 모델(Two-Tower MLP)**을 분산학습하는 패턴을 **실행 방식 × 컴퓨트 토폴로지** 매트릭스로 정리한 쿡북.

## 🎯 누구를 위한 쿡북인가

- PyTorch 추천 모델을 처음 Databricks에서 분산학습하려는 ML 엔지니어
- 노트북 PoC → 스크립트 기반 운영 → multi-node 스케일아웃 경로를 한 곳에서 보고 싶은 팀
- 같은 모델을 노트북 / 스크립트 / Accelerate / Lightning 중 어떤 방식으로 돌릴지 비교하려는 팀
...
| Lightning | PyTorch Lightning `Trainer(devices, num_nodes, strategy="ddp")` API로 분산화. |
...
- [`99-references/`](99-references/) — 공식 노트북 미러 및 외부 자료 인덱스 (LLM fine-tuning 예제 포함, 본 쿡북은 추천 도메인으로 일반화).

## ⚠️ 본 쿡북의 스코프

- **다룬다**: Two-Tower MLP 추천 모델, DDP, TorchDistributor, Accelerate, Lightning, multi-node 데이터 병렬.
- **다루지 않는다**: FSDP·DeepSpeed (모델 샤딩 불필요), LLM fine-tuning, TorchRec 임베딩 샤딩, vision/multimodal.
- **목표 학습 시간**: 각 셀 ≤ 15분 (합성 데이터 사용).
