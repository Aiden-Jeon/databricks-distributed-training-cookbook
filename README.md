...
|------------------|------------------|-------------------|---------------|
| **01-notebook-based** | [`02-train_torch_distributor.ipynb`](01-notebook-based/02-train_torch_distributor.ipynb) | [`03-train_pytorch_lightning.ipynb`](01-notebook-based/03-train_pytorch_lightning.ipynb) | — (CLI 기반이라 N/A) |
| **02-script-based** | [`02-launch_torch_distributor.ipynb`](02-script-based/02-launch_torch_distributor.ipynb) | [`03-launch_lightning_trainer.ipynb`](02-script-based/03-launch_lightning_trainer.ipynb) | [`04-launch_accelerator.ipynb`](02-script-based/04-launch_accelerator.ipynb) |
...
각 학습 노트북 안에서 토폴로지(1×1 GPU / 1×N GPU / M×N GPU)를 **섹션**으로 비교한다.
...
- 데이터: 합성 user-item interaction → Delta Lake → torch DataLoader
...
2. 매트릭스에서 자신의 시나리오에 가장 가까운 셀(노트북)을 연다.
3. 해당 행(`01-notebook-based` / `02-script-based` / `03-custom-package-script-based`)의 README "실행 순서"를 따른다.
4. 토폴로지 섹션을 1×1 → 1×N → M×N 순으로 확장한다.
...
- **다룬다**: Two-Tower MLP 추천 모델, DDP, TorchDistributor, Lightning, Accelerate, multi-node 데이터 병렬.
- **다루지 않는다**: FSDP·DeepSpeed (모델 샤딩 불필요), LLM fine-tuning, TorchRec 임베딩 샤딩, vision/multimodal, MosaicML Composer, Ray Train.
- **목표 학습 시간**: 각 노트북 섹션 ≤ 15분 (합성 데이터 사용).
